...
while True:
    ...
    button = grovepi.digitalRead(3)
    ultra = grovepi.ultrasonicRead(4) # raw distance
    ########################################################################################################
    # Step 2: use filter to pre-process raw data
	...
	# Method : Button
    action = action + button_last # when there is no action

    if action == 1: # present: Open the door, start
        unit_mode = 1 # 'Unlocked'
        before_distanceA = ultra_lastA
    elif action == 2: # present: Close the door, complete
        unit_mode = 0 #'Locked'
        ...
        action = 0 # action return the value to 0
    else: # action == 0
        unit_mode = mode_last
        n_itemsA = n_lastA
        changeA = 0
